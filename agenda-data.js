(() => {
  const empty = () => ({ schemaVersion: 2, selected: [], agenda: {}, priorities: {}, commitments: [], lodging: { address: "" } });
  ...
  const normalize = (input) => {
    ...
    result.priorities = Object.fromEntries(Object.entries(input.priorities || {}).filter(([id, score]) => result.selected.includes(id) && Number.isInteger(score) && score >= 0 && score <= 10));
